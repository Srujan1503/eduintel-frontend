...
export const Topbar = ({ onToggle }: TopbarProps) => {
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/70 bg-white/80 px-4 py-4 backdrop-blur sm:px-5 dark:border-slate-800 dark:bg-slate-950/80">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => void handleLogout()} className="rounded-2xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">Logout</button>
        <Link to="/profile" className="flex items-center gap-2 rounded-2xl bg-violet-600 px-3 py-2 text-sm font-semibold text-white shadow-lg shadow-violet-600/20">
          <Sparkles size={16} />
